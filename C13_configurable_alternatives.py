...
prompt = PromptTemplate.from_template("用繁體中文像朋友聊天回應我這段話：{topic}").configurable_alternatives(
    ...
    wiseman=ChatPromptTemplate.from_messages([
        ("system", "你是一個睿智且愛說故事的老人"),
        ("human", "用繁體中文回應我的這段話：{topic}")
    ]),
)
...
# ----------------------------
print("🟢 預設組合（friendly + gemma3）：")
print(chain.invoke({"topic": "請問如何規劃人生方向？"}))
...
print("\n🔵 切換組合（ formal + gemma3 ）：")
print(chain_custom.invoke({"topic": "請問如何規劃人生方向？"}))
